...
const TARGET = 300;   // IR 표준 템플릿 배분 합계 5:00
...
md.push(`> 발표본 **${D.S.length}장** · 실측 **${mmss(TOTAL)}** · IR 표준 배분 ${mmss(TARGET)} 대비 **${TOTAL - TARGET > 0 ? '+' : ''}${TOTAL - TARGET}초**`);
md.push(`> 이번 발표에서 뺀 슬라이드 ${D.OFF.length}장(${D.OFF.map((s) => s.title.replace('[보관] ', '')).join(' · ')})은 \`deck/deck.js\` 에 그대로 남아 있습니다.`);
...
md.push('- **S8에서 “갈 수 없는 자리는 광고를 사도 뜨지 않는다”는 반드시 말하십시오.** 광고 모델을 꺼내는 순간 들어오는 질문을 미리 막는 문장입니다.');
md.push('- **뺀 슬라이드(팀 · 요청 · 트랙션 · 백업)는 질문으로 받으십시오.** 답은 `넛츠_예상질문_답변.docx` 에 있습니다.');
