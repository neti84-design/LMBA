...
function size3(x, y, w, tag, title, value, note, ratio, o = {}) {
  ...
  return [
    ...
    rect(x + 0.28, y + 0.22, 0.86, 0.3, { fill: hot ? C.gold : C.shell, radius: 0.05 }),
    text(x + 0.28, y + 0.27, 0.86, 0.25, tag, { size: 10.5, bold: true, color: hot ? C.shell : C.white, align: 'center' }),
    text(x + 1.3, y + 0.24, w - 3.25, 0.28, title, { size: 11.5, bold: true, color: C.shell }),
...
add('문제 — 두 개의 빈칸', 60, [
  ...
  text(M, 4.55, 5.5, 0.3, 'PROBLEM 01  ·  사장님', { size: 11.5, bold: true, color: C.gold, space: 1.5 }),
  ...
  text(M + 6.13, 4.55, 5.5, 0.3, 'PROBLEM 02  ·  사용자', { size: 11.5, bold: true, color: C.gold, space: 1.5 }),
...
add('솔루션', 40, [
  ...
  text(M + 0.3, 5.04, 6.0, 0.28, '도달 가능성 필터 — 현재시각 + 이동시간 + 준비버퍼 ≤ 예약 마감시각 (도보 · 자전거 · 차량)', { size: 10.5, color: C.gold }),
...
add('코어 메커니즘', 20, [
  rect(0, 0, W, H, { fill: C.cream }),
  ...head('CORE MECHANISM', '코어는 빈 시간을 지금 예약하게 만드는 것입니다'),
  text(M, 1.42, CW, 0.35, '중심 엔진은 다이나믹 프라이싱이고, 도달 가능성은 값이 붙은 슬롯을 실제로 살 수 있는 사람에게 잇는 필터입니다.', { size: 14, color: C.grey }),

  ...mech(M + 0 * 1.9755, 1.95, 1.7555, 2.35, '01', '빈 슬롯 발생', '당일 취소 · 비수요 시간대 · 예약 공백'),
  ...mech(M + 1 * 1.9755, 1.95, 1.7555, 2.35, '02', '소멸 임박 판단', '시간이 지나면 이 슬롯은 0원이 된다'),
  ...mech(M + 2 * 1.9755, 1.95, 1.7555, 2.35, '03', '가격 조정', '남은 시간 · 수요 밀도 · 할인율로 재설정'),
  ...mech(M + 3 * 1.9755, 1.95, 1.7555, 2.35, '04', '도달 가능 수요에 노출', '실제 갈 수 있는 사람에게만 보여줌'),
  ...mech(M + 4 * 1.9755, 1.95, 1.7555, 2.35, '05', '즉시 선결제', '노쇼를 줄이고 사장님 매출 확정'),
  ...mech(M + 5 * 1.9755, 1.95, 1.7555, 2.35, '06', '재방문 전환', '할인 체험을 정가 고객으로 전환'),

  rect(M, 4.62, CW, 1.18, { fill: C.shell, radius: 0.08 }),
  text(M + 0.5, 4.86, CW - 1.0, 0.75, '넛츠는 서비스업의 비어 있는 시간 슬롯을 마감시간 · 수요밀도 · 도달가능성에 따라 실시간으로 가격화하고,\n즉시 구매 가능한 고객에게 연결하는 서비스형 다이나믹 프라이싱 플랫폼입니다.', { size: 15.5, bold: true, color: C.white, align: 'center', lineSpacing: 1.45 }),
...
add('포지셔닝 맵', 15, [
  rect(0, 0, W, H, { fill: C.cream }),
  ...head('MARKET · POSITIONING', '인접 앱은 다 있습니다. 합쳐진 자리만 비어 있습니다'),
  ...
  text(M + 0.18, 4.2, 1.7, 0.26, '←  물건 · 음식', { size: 10.5, bold: true, color: '9B8873' }),
  text(6.55, 4.2, 1.7, 0.26, '서비스업의 시간  →', { size: 10.5, bold: true, color: '9B8873' }),

  ...plot(2.15, 2.78, '마감히어로 · 라스트오더'),
  ...plot(3.45, 3.45, '당근마켓'),
  ...plot(2.40, 5.30, '배달 · 예약 주문'),
  ...plot(5.95, 5.42, '네이버예약 · 카카오헤어샵'),
  ...plot(7.35, 4.80, '캐치테이블'),
  ...plot(6.60, 2.95, '넛츠', { hot: true, note: '오늘 비어버린 시간 · 지금' }),

  /* 오른쪽 비교 */
  ...compare(8.75, 1.95, 3.733, 0.82, '네이버예약 · 카카오헤어샵', '내일과 다음 주를 정가로 팝니다'),
...
add('타깃 · 시장 규모', 15, [
  ...
  text(M, 1.42, CW, 0.35, '시장 수치는 공개 통계와 업계 통상치를 근거로 한 가정값입니다. 내부 데이터로 검증 후 확정합니다 [검증필요]', { size: 12.5, color: C.grey }),
  ...
  ...size3(M + 6.13, 1.95, 5.5, 'TAM', '예약 기반 개인 서비스업', '약 15조', '미용 · 네일 · 마사지 · 에스테틱 등 국내 연간 거래액 [검증필요]', 1.0),
  ...size3(M + 6.13, 3.35, 5.5, 'SAM', '소멸하는 빈 슬롯', '약 4조', '예약 가동률 갭 25~30% 가정 — 매일 0원으로 사라지는 공급 [검증필요]', 0.27),
  ...size3(M + 6.13, 4.75, 5.5, 'SOM', '3년 내 서울 5개 구', '약 200억', 'SAM의 0.5% · 강남구에서 시작해 서울 주요 5개 구까지', 0.05, { hot: true }),
...
add('클로징', 10, [
  ...
  text(M, H - 0.85, 7, 0.3, '넛츠  NUTS   ·   감사합니다', { size: 11.5, color: 'A8927B', space: 1.5 }),
], [
