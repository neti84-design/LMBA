...
S(1, '두 사람', '1:00',
  '좌우 2분할. 왼쪽: 카페 창가에 혼자 앉은 사람의 손(손톱 클로즈업). 오른쪽: 텅 빈 네일샵 의자 하나. 자막 없음.',
  ['여기 두 사람이 있습니다. /',
  ...
  '"이미 있지 않냐고 물으실 겁니다" → S2');

c.push(BREAK());
S(2, '이 조합이 시장에 없다', '1:15',
  '2×2 매트릭스. 가로축 = 무엇을 파나(음식 ↔ **서비스업**), 세로축 = 언제 파나(미리 예약 ↔ **지금, 내 주변**). 세 칸에 마감히어로 / 당근 / 네이버예약이 들어가고 **우상단 한 칸만 비어 있다.** 마지막에 그 칸이 채워진다.',
...
c.push(BREAK());
S(4, '도달 계산 — 내 주변을 진짜 갈 수 있는 곳으로', '1:12',
  ...
  '화면을 그대로 둔 채 "이 작은 장치가 세 가지를 만듭니다" → S5',
  '★ 이 슬라이드가 그 주장의 증거입니다. 화면을 보고 말하십시오. "사라집니다"라고 할 때 실제로 항목이 사라지는 타이밍에 맞추십시오.');
...
S(7, '확장', '0:53',
  '4칸 사다리(서비스업 → 문화 → 숙박 → 포장음식). 1·4단계는 소상공인 색, 2·3단계는 대기업 색. 오른쪽에 화살표: `첫 문 → 롯데시네마 · 롯데호텔`',
  ['확장은 네 단계입니다. /',
...
S(9, '수익 — 언제, 얼마', '1:02',
  ...
  null,
  '숫자를 외워서 읊으면 지어낸 것처럼 들립니다. 아래 표를 손으로 짚으며 말하십시오.');

c.push(GAP());
c.push(TBL(['억 원', '1년차', '2년차', '3년차', '4년차'], [
  ['거래액 (GMV)', '5', '43', '202', '576'],
  ['매출', '0.3', '4.1', '22.2', '74.9'],
  ['영업손익', { t: '-4.7', color: RED }, { t: '-8.1', color: RED }, { t: '**+2.2**', color: '2E6B3E' }, { t: '**+39.9**', color: '2E6B3E' }],
  ['누적 손익', '-4.7', { t: '**-12.8**', color: RED }, '-10.6', { t: '**+29.3**', color: '2E6B3E' }],
...
c.push(P('**S2 · S4 · S9 · S12는 어떤 경우에도 건드리지 않습니다.** S2가 이 발표의 주장이고, S4가 그 주장의 증거입니다.'));
...
c.push(GAP());
c.push(P('전부 빼면 **8:00.** 여유가 59초 있으니 실제로는 1~2번만 준비해 두면 충분합니다.'));
...
c.push(LI('**S2가 이 발표의 주장입니다.** 마감히어로 · 당근 · 네이버예약을 하나씩 짚으며 "그런데 음식만", "그런데 중고 물건", "그런데 내일과 다음 주"를 **같은 리듬으로 세 번** 반복하십시오. 빈칸이 귀로 들립니다. 그리고 **"왜 비어 있는가"까지 답하고 넘어가십시오** — 여기서 멈추면 "운 좋게 남은 자리"로 들리고, 끝까지 가면 "구조적으로 아무도 못 오는 자리"가 됩니다'));
c.push(LI('**S3는 짧게, 화면만 보여주고 넘어가십시오.** 여기서 설명을 길게 하면 S4의 데모가 김이 빠집니다'));
c.push(LI('**S4에서는 화면을 보고 말하십시오.** 청중이 화면을 봐야 하는 유일한 슬라이드입니다. 도보에서 "사라집니다"라고 할 때 실제로 항목이 사라지는 타이밍에 맞추십시오'));
c.push(LI('**S7은 빠르게.** 네 단계는 이름만 스치듯 지나가면 됩니다. 순서의 이유를 물으면 그때 답하십시오 — 문서에 다 있습니다'));
...
c.push(LI('입으로 말하는 숫자는 **데모에서 4개**(38분 · 2.5킬로 · 47명 · 112명), **재무에서 5개**(5억 · 43억 · 200억 · 75억 · 13억)뿐입니다'));
...
const QA = [
  ...
  ['Q3. 그 재무 숫자의 근거가 뭡니까?',
   '시장 규모에서 역산한 게 아니라 **단위에서 쌓아 올린 숫자**입니다. 변수는 네 개뿐입니다. 입점 매장 수, 매장당 월 성사 슬롯, 평균 객단가, 수수료율. 이 중 가장 불확실한 건 **매장당 월 성사 슬롯**이고, 1년차에 저희가 검증하려는 것도 정확히 그 하나입니다. 월 8건이 안 나오면 나머지 숫자는 전부 다시 써야 한다고 보고 있습니다.'],
