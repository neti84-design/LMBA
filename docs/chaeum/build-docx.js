...
const toc = [
  ...
  ['부록 B', '리스크 자문 의견 (상세)'],
  ['부록 C', '10분 발표 대본 (15장) + 발표 운영 노트 · Q&A'],
  ['부록 D', '후속 액션'],
];
...
c.push(P('기준 속도 **300자/분** (발표용 호흡, 문장 사이 쉼 포함). 대본 안의 **/** 는 한 박자 쉬는 지점, **굵은 글씨**는 힘주어 말할 지점입니다.', { size: 18, color: GREY }));
...
SC(1, '두 사람  (0:44)',
  '좌우 2분할. 왼쪽은 카페 창가에 혼자 앉은 사람의 손(손톱 클로즈업), 오른쪽은 텅 빈 네일샵 의자 하나. 자막 없음.',
  ['여기 두 사람이 있습니다. /',
  ...
  '화면 중앙에 선이 그어지며 두 이미지가 갈라진다');

SC(2, '두 개의 빈칸  (0:38)',
  '좌: 사장님 타임테이블의 13~14시 칸만 회색으로 빔. 우: 사용자 하루 일정의 14~16시가 빔. 가운데는 끊어진 선.',
  ['이 장면에는 빈칸이 두 개 있습니다. /',
...
SC(3, '왜 안 만나는가  (0:29)',
  '4열 표. 네이버예약·카카오헤어샵 / 마감히어로 / 당근 / 채움. 행은 "다루는 시간축"과 "다루는 재고". 채움 칸만 비었다가 채워짐.',
  ['왜 안 만날까요. 기존 서비스들이 **서로 다른 시간을 다루기** 때문입니다. /',
   '네이버예약과 카카오헤어샵은 내일과 다음 주를 답니다. 미래의 예약이죠. 마감히어로 같은 타임세일 앱은 오늘을 다루지만, 음식만 다룹니다. 당근은 지역을 잇지만 중고 물건입니다. /',
   '**지금부터 두 시간, 그리고 서비스업.** 이 칸이 비어 있습니다.'],
  '빈 칸에 채움 로고가 들어감');

SC(4, '채움  (0:19)',
  '검은 배경에 흰 글씨 한 줄. "지금, 내가 갈 수 있는 거리 안에서, 비어 있는 예약 한 자리를 채운다."',
  ['저희가 만들려는 것이 이 빈칸입니다. 이름은 **채움**입니다. /',
   '지금, 내가 갈 수 있는 거리 안에서, 비어 있는 예약 한 자리를 채운다. /',
   '사장님의 빈 시간과 손님의 빈 시간을 **같이** 채운다는 뜻입니다.'],
  '"그런데 여기서 가장 중요한 질문이 하나 남습니다" 하며 넘어감');

c.push(H3('★ 다음 슬라이드가 이 발표의 심장입니다. 애니메이션에 가장 공을 들이십시오.', RED));
SC(5, '핵심 데모 — 이동수단 필터  (1:17)',
  '3단 애니메이션. ① 사장님 등록 화면 ② 손님 화면의 마감 카운트다운 ③ 도보/자전거/차량 버튼을 누를 때마다 목록이 바뀌는 장면.',
...
SC(9, '확장 전략 — 아마존의 순서  (1:28)',
  '상단은 아마존 사다리(책 → 음반 → 전자제품 → 신선식품), 하단은 채움 사다리(서비스업 → 문화 → 숙박 → 포장음식). 같은 색으로 1:1 대응.',
...
SC(12, '수익 모델  (0:33)',
  '3단 타임라인. 수수료 0% → 광고 → 채움페이 · 인증 배지. 아래에 각 단계의 전환 조건.',
...
SC(13, '로드맵과 요청  (0:34)',
  '좌: 타임라인 3단(M3 / M6 / M12). 우: 큰 글씨로 North Star "채워진 슬롯 수". 하단에 요청 2줄.',
...
c.push(TBL(['슬라이드', '누적', '분량', '비고'], [
  ...
  [{ t: '**S5 핵심 데모**', color: RED }, { t: '2:12', color: RED }, { t: '**1:17**', color: RED }, { t: '**발표의 심장. 절대 줄이지 말 것**', color: RED }],
  ['S6 세 가지 효과', '3:30', '0:28', ''],
  ['S7 사장님 화면', '3:58', '0:23', ''],
  ['S8 경쟁 지도', '4:22', '0:20', ''],
  ['S9 확장 — 아마존', '4:43', '1:28', '두 번째로 중요한 슬라이드'],
  ['S10 소상공인 → 대기업', '6:11', '0:38', ''],
  ['S11 운영 3원칙', '6:50', '0:38', ''],
  ['S12 수익 모델', '7:28', '0:33', ''],
  ['S13 로드맵과 요청', '8:02', '0:34', '요청 문장은 또박또박'],
  ['S14 두 가지 지적', '8:36', '0:35', ''],
  ['S15 클로징', '9:12', '0:37', '마지막 2초 침묵'],
  [{ t: '**합계**', color: NAVY }, '', { t: '**9:49**', color: NAVY }, { t: '10:00까지 여유 10초', color: NAVY }],
], [2300, 1100, 1100, 5138]));

c.push(H2('시간이 밀릴 때 버릴 순서'));
c.push(P('실전에서는 거의 항상 밀립니다. **아래 순서대로, 표시된 문장만** 통째로 빼십시오. **S5·S9·S15는 어떤 경우에도 건드리지 않습니다.**'));
...
c.push(GAP());
c.push(P('**전부 빼면 8:49. 1~3번만 빼도 9:15로 안전권입니다.**'));

c.push(H2('전달 지침'));
c.push(LI('**S1은 낭독이 아니라 묘사입니다.** "손톱 정리를 한 게 언제였더라"에서 실제로 자기 손을 한 번 내려다보십시오. 청중이 같이 봅니다'));
c.push(LI('**"/" 는 한 박자입니다.** 특히 S1의 마지막 두 줄 사이는 두 박자. "두 사람은 서로의 존재를 모릅니다" 다음의 침묵이 이 발표에서 가장 비싼 2초입니다'));
c.push(LI('**S5에서는 화면을 보고 말하십시오.** 청중이 화면을 봐야 하는 유일한 슬라이드입니다. 도보에서 "사라집니다"라고 할 때 실제로 항목이 사라지는 타이밍에 맞추십시오'));
c.push(LI('**S9는 리듬입니다.** 1·2·3·4단계를 같은 억양으로 네 번 반복하면 사다리가 귀로 들립니다'));
c.push(LI('**S13의 마지막 문장(요청)은 천천히.** 상대가 기억해야 할 유일한 행동 항목입니다'));
c.push(LI('**S15 "감사합니다" 전에 2초 멈추십시오.** "인프라"라는 단어가 착지할 시간을 주십시오'));
c.push(LI('숫자는 **38분 · 2.5킬로 · 47명 · 112명** 네 개만 입으로 말합니다. 나머지는 슬라이드에만 두십시오'));

c.push(H2('Q&A 대비 — 예상 질문 5개와 30초 답변'));
const QA = [
