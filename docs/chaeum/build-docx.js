...
c.push(H2('8.2 4개년 재무 추정'));
c.push(...BOX(['시장 규모에서 역산하지 않고 **단위 경제에서 쌓아 올린 추정치**입니다.',
               '전부 가정이며, 1년차의 검증 대상은 **"가맹당 월 성사 슬롯"** 하나입니다.'], NAVY));
c.push(H3('입력 가정'));
c.push(TBL(['변수', '1년차', '2년차', '3년차', '4년차'], [
  ...
  ['**가맹당 월 성사 슬롯**', { t: '**8건**', color: RED }, '12건', '14건', '16건'],
  ['평균 객단가', '25,000원', '25,000원', '30,000원', '30,000원'],
  ['거래액 대비 수익률', { t: '**0%**', color: NAVY }, '5%', '8%', '9%'],
  ['인원', '5명', '15명', '25명', '40명'],
], [2400, 1810, 1810, 1810, 1808]));
c.push(GAP());
c.push(P('**수수료는 전 기간 0%입니다.** 위 "수익률"은 광고·채움페이·인증 배지 수익을 거래액 대비로 환산한 값입니다.'));
...
c.push(TBL(['', '1년차', '2년차', '3년차', '4년차'], [
  ...
  ['**영업손익**', { t: '-5.0', color: RED }, { t: '-10.0', color: RED }, { t: '-3.9', color: RED }, { t: '**+16.8**', color: GREEN }],
  ['**누적 손익**', '-5.0', '-15.0', { t: '**-18.9**', color: RED }, '-2.1'],
], [2400, 1810, 1810, 1810, 1808]));
c.push(GAP());
c.push(LI('**월 단위 흑자 전환**: 3년차 4분기'));
c.push(LI('**연 단위 흑자 전환**: 4년차'));
c.push(LI('**필요 투자액** (최대 누적 적자): 약 **20억 원** — 3년차 말'));
c.push(LI('**누적 회수**: 5년차 초'));
...
c.push(NLI('**가맹당 월 8건** — 가장 취약합니다. 4건이면 1년차 거래액이 반토막 나고 흑자 전환이 1년 밀립니다'));
c.push(NLI('**거래액 대비 수익률 5→9%** — 광고 단가가 검증되지 않은 값입니다. 3%에 그치면 4년차 매출이 52억에서 17억으로 떨어집니다'));
c.push(NLI('**가맹 확보 속도** — 소상공인 1:1 영업은 인력에 선형으로 묶입니다. 2년차 2,000개는 영업 인력 확보가 전제입니다'));
c.push(GAP());
...
SC(9, '수익 — 언제, 얼마  (0:52)',
  '4개년 표(거래액 / 매출 / 영업손익 / 누적)와 꺾은선 하나. 3년차 4분기에 0선을 통과하는 지점을 굵게 표시. 근거는 8.2절.',
