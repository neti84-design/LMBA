...
c.push(new Paragraph({ children: [], spacing: { after: 1700 } }));
c.push(new Paragraph({
  children: [new ImageRun({ type: 'png', data: fs.readFileSync(LOGO),
                            transformation: { width: 78, height: 127 } })],
...
c.push(new Paragraph({
  children: [new TextRun({ text: '넛츠', font: FONT, size: 88, bold: true, color: SHELL })],
  alignment: AlignmentType.CENTER, spacing: { after: 100 },
}));
c.push(new Paragraph({
  children: [new TextRun({ text: 'NUTS', font: FONT, size: 22, color: GOLDINK, characterSpacing: 120 })],
...
c.push(P('넛츠는 소상공인의 **비어버린 예약 슬롯**과 사용자의 **비어버린 시간**을 서로 채워주는, 위치·시간·**이동수단** 기반 실시간 예약 마켓플레이스입니다.'));
c.push(P('기존 예약 서비스가 "내일 몇 시"를 다룬다면, 넛츠는 **"지금부터 두 시간 안"** 만 다룹니다. 그리고 그 안에서도 **내가 실제로 도착할 수 있는 곳**만 보여줍니다.'));

c.push(H2('1.1 이름 — 넛츠 (NUTS)'));
c.push(P('**심심풀이 땅콩.** 심심풀이는 남는 시간을 때우는 것을 뜻합니다. 비유가 아니라 **이 서비스의 수요 정의 그 자체**입니다 — 약속 전 30분, 늦는 친구, 취소된 회의. 관용구가 이미 "무료한 시간을 채우는 것"을 뜻하고 있으므로, 이름이 의미를 지고 오는 게 아니라 **한국어가 이미 지고 있던 의미 위에 올라탑니다.**'));
c.push(P('견과류의 물성도 제품과 맞습니다. 한 줌, 짧게, 부담 없이 — **30분~2시간짜리 가벼운 소비와 같은 크기감**입니다. 코스 요리가 아니라 집어먹는 것.'));
c.push(TBL(['글자', '단어', '뜻'], [
...
c.push(GAP());
c.push(P('이름은 **역선택 리스크(부록 B의 R-A)** 를 한 겹 막아주기도 합니다. "빈 슬롯 파는 가게는 안 되는 가게 아니냐"는 결국 **떨이 프레임** 문제인데, 심심풀이는 떨이가 아닙니다. 싸서 집는 게 아니라 **심심해서** 집는 것이기 때문입니다.'));
c.push(P('다만 **프레임은 대상에 따라 분리합니다.** 소비자에게는 "심심한 시간에, 한 줌, 가볍게", 사장님에게는 "안 팔리면 0원인 슬롯을 매출로". 사장님 대상 문구에 심심풀이가 들어가면 재고 가치를 스스로 깎는 꼴이 됩니다.', { size: 18, color: GREY }));
...
c.push(TBL(['서비스', '다루는 시간축', '다루는 재고'], [
  ...
  ['당근', '상시', '중고 물건'],
  [{ t: '넛츠', color: NAVY }, { t: '**지금부터 2시간 이내**', color: NAVY }, { t: '**서비스업의 빈 슬롯**', color: NAVY }],
], [3200, 3400, 3038]));
c.push(GAP());
c.push(...BOX(['넛츠가 노리는 빈칸 = **지금(Now) × 서비스업 × 실제로 갈 수 있는 거리**']));
...
c.push(P('넛츠는 반대입니다. **갈 수 있는 것만 보여줍니다.**'));
...
c.push(H2('3.3 "넛츠 반경" = 남은 시간 × 이동수단 속도'));
...
c.push(GAP());
c.push(...BOX(['같은 시각, 같은 자리에 있어도 **이동수단에 따라 완전히 다른 화면을 본다.**', '이것이 넛츠의 단일 핵심 메커니즘입니다.'], NAVY));
...
c.push(TBL(['서비스', '카테고리', '시간축', '할인', '도달가능성 필터'], [
  ...
  ['당근', '중고거래', '상시', '—', '없음'],
  [{ t: '**넛츠**', color: NAVY }, { t: '**서비스업**', color: NAVY }, { t: '**지금 (2시간 내)**', color: NAVY }, { t: '**있음**', color: NAVY }, { t: '**핵심 기능**', color: NAVY }],
...
c.push(P('넛츠의 재고는 물건이 아니라 **시간**입니다. 그리고 시간 재고에도 똑같은 난이도 사다리가 있습니다.'));
c.push(TBL(['아마존', '넛츠', '공통 원리'], [
...
c.push(GAP());
c.push(P('즉흥 소비와의 궁합도 가장 좋습니다. **"20분 뒤 시작하는 회차", "지금 비어 있는 방탈출 한 타임"** 은 넛츠의 도달 필터가 가장 아름답게 작동하는 지점입니다.'));
...
c.push(H2('6.6 4단계: 포장음식 — 넛츠의 "신선식품"'));
...
c.push(P('넛츠는 소상공인으로 시작하지만 **소상공인 플랫폼으로 끝나지 않습니다.**'));
...
c.push(P("당초 '당근처럼 초기 무수수료'를 검토했으나 **철회를 권고합니다.** 당근과 넛츠는 구조가 다릅니다."));
c.push(TBL(['', '당근', '넛츠'], [
...
c.push(TBL(['단계', '시점', '모델', '설명'], [
  ...
  ['**Phase 3**', '25개월~', '**+ 넛츠페이**', '결제 수수료 + 예치금 운용 수익 + **락인(lock-in) 효과**'],
...
c.push(...BOX(['**확장 원칙 한 줄**', '"시간이 재고인가, 그리고 초면인 사람을 지금 받을 수 있는가."',
               '두 질문에 모두 "예"면 넛츠에 들어오고, 하나라도 "아니오"면 다음 단계로 미룹니다.'], NAVY));
...
c.push(P('기준 속도 **300자/분** (발표용 호흡 포함). 대본 안의 **/** 는 한 박자 쉬는 지점, **굵은 글씨**는 힘주어 말할 지점입니다. 총 **12장** — **시장의 빈칸(S2) → 무엇을 파는가(S3) → 어떻게 작동하는가(S4)** 순서입니다.', { size: 18, color: GREY }));
...
c.push(H3('★ 이 슬라이드가 이 발표의 주장입니다. "왜 아직 아무도 없나"까지 답하고 넘어가십시오.', RED));
SC(2, '이 조합이 시장에 없다  (1:15)',
...
c.push(H3('★ 이 슬라이드가 그 주장의 증거입니다. 애니메이션에 가장 공을 들이십시오.', RED));
SC(4, '도달 계산 — 내 주변을 진짜 갈 수 있는 곳으로  (1:12)',
...
c.push(GAP());
c.push(P('전부 빼면 **8:16.** 여유가 44초 있으니 실제로는 1~2번만 준비해 두면 충분합니다.'));
...
Packer.toBuffer(doc).then(b => {
  fs.writeFileSync(require('path').join(__dirname, '..', '넛츠_신사업_제안서.docx'), b);
